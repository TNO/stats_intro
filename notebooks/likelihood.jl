...
_Informal definition_: An estimator gives a guess about the true state (parameter, $\boldsymbol{\theta}$) of nature.
"""
...
begin
	...
	# Population
	pop_distr = Normal(pop_mean, pop_std)
	radius = rand(pop_distr, n_pop_row^2)
	x = 1:n_pop_row
	y = 1:n_pop_row
	x_mx = x' .* ones(n_pop_row)
	y_mx = ones(n_pop_row)' .* y
end;
...
begin
	...
	# Sample
	# ........................
	sample_idx = sample(1:(n_pop_row^2), n_sample, replace=false)
	sample_x = x_mx[:][sample_idx]
	sample_y = y_mx[:][sample_idx]
	sample_radius = radius[sample_idx]
	...
	# ........................
	begin
		p_pop = scatter(x_mx[:], y_mx[:], markersize=radius[:],
			label=nothing, axis=([], false), title="Population",
		)
		scatter!(sample_x, sample_y, markersize=sample_radius,
			label=nothing, markercolor="red",
		)
...
_Informal definition_: The mathematical model that is assumed to generated the data. In statistics it should contain a probabilistic component.
"""
...
Usually it is more complex, for example a common linear model:
...
__Data generating process/model $\rightarrow$ likelihood function__
"""
